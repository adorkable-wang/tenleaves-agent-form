...
export function extractPayload(content: DashscopeMessage['content']): Record<string, unknown> {
  if (!content) return {}
  ...
  if (Array.isArray(content)) {
    ...
    const parsed = extractJsonFromText(concatenated) ?? tryParseJSON(concatenated)
    if (parsed && typeof parsed === 'object') {
      return ensureStructure(parsed as Record<string, unknown>)
    }
...
function tryParseJSON(text: string): JsonLike | undefined {
  try {
...
function extractJsonFromText(text: string): JsonLike | undefined {
...
function findFirstJsonSlice(input: string): string | null {
  ...
  const startIdx =
    idxBrace === -1 ? idxBracket : idxBracket === -1 ? idxBrace : Math.min(idxBrace, idxBracket)
