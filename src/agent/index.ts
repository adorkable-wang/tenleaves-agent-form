...
const resolvedEndpoint =
  typeof import.meta.env.VITE_AGENT_ENDPOINT === 'string' &&
  import.meta.env.VITE_AGENT_ENDPOINT.trim().length
    ? import.meta.env.VITE_AGENT_ENDPOINT
    : undefined

const resolvedApiKey =
  typeof import.meta.env.VITE_AGENT_API_KEY === 'string' &&
  import.meta.env.VITE_AGENT_API_KEY.trim().length
    ? import.meta.env.VITE_AGENT_API_KEY
    : undefined

const sharedConfig = {
  endpoint: resolvedEndpoint ?? '/api/agent/analyze',
  apiKey: resolvedApiKey,
} satisfies RemoteAgentConfig
const sharedBackend: AgentBackend = new RemoteAgentBackend(sharedConfig)
